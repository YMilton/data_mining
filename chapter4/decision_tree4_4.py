...
'''
Branching for decision tree use recursion(使用递归划分决策树)
'''
def TreeGenerate(df):
    ...
    if label_count:
        ...
        # get the optimal attr for a new branching, via Gini index
    ...
    else: # left and right child (continous variable)
...
'''
make a predict based on root (基于根节点做预测)
...
'''
calculate accuracy of prediction on test set (在测试集上计算预测准确率)
    @:param root: Node, root Node of the decision tree
...
'''
pre_purning to generate a decision tree (预剪枝生成决策树)
    @:param df_train: dataframe, the training set for purning decision
...
def PrePurn(df_train, df_test):
    ...
    if label_count:  # assert the label_count isn't empty
        ...
        # end if there is only 1 class in current node data or attribution array is empty
        if len(label_count) == 1 or len(label_arr) == 0:
            return new_node
...
'''
find the optimal attributes of current data_set based on gini index (通过gini指数查找当前数据集的最优属性)
    @:param df: dataframe of the data_set
    @:return opt_attr: the optimal attributes for branch
...
'''
calculate the information gain of an attribute (计算信息链的属性)
    @:param df: dataframe, the dataframe of the data set
    @:param attr_id: the target attribute in df
'''
...
'''
calculate the information entropy of an attribute (计算属性的信息熵)
    @:param label_arr: class label array of data_arr
'''
...
'''
calculate the gini index of an attribute (计算一个属性的基尼指数)
    @:param df: dataframe, the pandas dataframe of the data set
    @:param attr_id: the target attribution in df
    @:return gini_index: the gini index of current attribution
    @:return div_value: for discrete variable, value=0,  for continous variable, value=t (the division value)
'''
...
'''
calculate the gini value of an attribute
    @:param label_arr: class label array of data_arr
    @:return gini: the information entropy of current attribute
'''
def Gini(label_arr):
    gini = 1
...
'''
visualization of decision tree from root (可视化决策树)
    @:param root: Node, the root node of tree
    @:param out_file: name and path of output file
'''
...
if __name__ == '__main__':
    df = pd.read_csv('data/watermelon_2.csv')
